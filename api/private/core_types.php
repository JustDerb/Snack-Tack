...
function st_types_getList()
{
	...
	//Check for record
	$query = "SELECT t.id,t.icon,c.id AS 'categoryid',c.name AS 'category',c.description AS 'categorydesc',t.name,t.description FROM `category` c, `types` t WHERE c.id=t.category ORDER BY c.id ASC";
